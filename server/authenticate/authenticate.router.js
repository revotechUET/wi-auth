...
function updateLastLoggedIn(idUser) {
  User.findByPk(idUser).then((u) => {
    u.last_logged_in = Date.now();
    u.save();
  });
}
  ...
  refreshTokenModel.checkRefreshToken(refreshToken, function (result) {
    if (result) {
          ...
          res
            ...
            .send(
              ResponseJSON(
                ...
                "You are not activated. Please wait for account activation.",
                "You are not activated. Please wait for account activation."
              )
            );
        } else {
          refreshTokenModel.renewRefreshToken(
            result.refreshToken,
            function (token, newRefreshToken) {
              let response = {};
              ...
              res
                .status(200)
                .send(
                  ResponseJSON(ErrorCodes.SUCCESS, "Successfull", response)
                );
            }
          );
        }
      ...
      res
        .status(200)
        .send(
          ResponseJSON(ErrorCodes.ERROR_WRONG_PASSWORD, "Session expired!")
        );
    }
  });
});
      ...
      User.findOne({ where: { username }, include: { model: models.Company } })
        .then(async function (user) {
          if (user) {
            ...
            if (
              logedin &&
              process.env.MULTI_LOGIN === "NotAllowed" &&
              req.body.client_id !== "sync-app"
            )
              await refreshTokenModel.clearTokenByUser(user.idUser);
            return done(null, {
              status: true,
              user: user,
            });
...
router.post(
  ...
  function (req, res) {
    if (!req.user.status)
      return res.send(
        ResponseJSON(
          ErrorCodes.ERROR_INVALID_PARAMS,
          req.user.message,
          req.user.message
        )
      );
    ...
    updateLastLoggedIn(user.idUser);
    if (
      req.body.whoami === "data-administrator-service" ||
      (req.body.whoami === "i2g-data-administrator" &&
        "" + parseInt(user.role) !== "3")
    ) {
      return res.send(
        ResponseJSON(
          ErrorCodes.ERROR_INVALID_PARAMS,
          "You are not alowed to login."
        )
      );
    }
    const data = {
    ...
    refreshTokenModel.createRefreshToken(
      ...
      function (refreshToken) {
        ...
        return res.send(
          ResponseJSON(ErrorCodes.SUCCESS, "Successful", response)
        );
      }
    );
    // }
...
router.post("/is-authenticated", (req, res) => {
  ...
  refreshTokenModel
    .findRefreshToken({ client_id: client_id })
    .then((r) => {
      if (!r) {
        res.send(
          ResponseJSON(ErrorCodes.SUCCESS, "Session dose not exist", {
            token: null,
            refreshToken: null,
          })
        );
        ...
        if (
          wi_client === "data-administrator-service" ||
          (wi_client === "i2g-data-administrator" &&
            "" + parseInt(decoded.role) !== "3")
        ) {
          return res.send(
            ResponseJSON(
              ErrorCodes.ERROR_INVALID_PARAMS,
            ...
            (session) => {
              if (!session)
                return res.send(
                  ResponseJSON(
                    ErrorCodes.ERROR_INVALID_PARAMS,
                    "Error while create new refresh token",
                    "Error while create new refresh token"
                  )
                );
              res.send(
                ResponseJSON(ErrorCodes.SUCCESS, "Session existed ", {
                  token: session.token,
    ...
    .catch((e) => {
      console.log(e);
      res.send(
        ResponseJSON(ErrorCodes.ERROR_INVALID_PARAMS, "Error " + e.message, e)
      );
    });
});
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let username =
        claims["upn"] || claims["unique_name"] || claims["preferred_username"];
      let userDomain = username.split("@").pop();
      let company = userDomain
        ? await companyModel.findCompanyByDomain(userDomain)
        : null;
      let userCreated = company
      ...
      if (user.status !== "Active") {
        res.redirect(
          "/auth-failed?message=" +
            "You are not activated. Please wait for account activation. Or contact us via this address support@i2g.cloud"
        );
      } else if (
        req.cookies.whoami === "data-administrator-service" ||
        (req.cookies.whoami === "i2g-data-administrator" &&
          "" + parseInt(user.role) !== "3")
      ) {
        ...
        let logedin = await checkUserLogedIn(user.idUser);
        if (logedin && process.env.MULTI_LOGIN === "NotAllowed")
          await refreshTokenModel.clearTokenByUser(user.idUser);
        const data = {
          username: user.username,
          role: user.role,
        ...
        refreshTokenModel.createRefreshToken(
          ...
          (session) => {
            if (!session)
              return res.redirect(
                "/auth-failed?message=" + "Can not create session"
              );
            res.redirect(
              `${process.env.AUTH_CLIENT_REDIRECT}?token=${session.token}&refreshToken=${session.refreshToken}&client_id=${session.client_id}`
            );
          }
        );
      }
